...
✅ ALLOWED: "Implement Oracle Engine search interface"
✅ ALLOWED: "Optimize relationship scoring algorithm"
❌ FORBIDDEN: "Build Oracle Engine and also add social features"
❌ FORBIDDEN: "Fix bugs and add new features and refactor architecture"

Rationale: Single focus ensures quality and prevents context dilution.
```

### **2. Revolutionary Standards Validation**
```markdown
EVERY suggestion must pass the 5-test framework:
...
❌ NEVER suggest features that fail ANY of these tests.
```

### **3. Backwards Compatibility Commitment**
```markdown
AI assistants MUST maintain backwards compatibility:
✅ Enhance existing features without breaking functionality
✅ Add new capabilities while preserving current workflows
✅ Improve performance without changing user experience
❌ Never break existing Oracle Engine functionality
❌ Never remove features without explicit strategic approval

Exception: Only with @exec (full Virtual Executive Team) approval.
```

### **4. Performance First Architecture**
```markdown
Oracle Engine <10 second response time is NON-NEGOTIABLE:
✅ Always consider performance impact of suggestions
✅ Recommend caching strategies for expensive operations
✅ Optimize database queries and API calls
✅ Use background processing for non-critical tasks
❌ Never suggest features that could slow Oracle responses
❌ Never recommend synchronous operations that block user experience

Every code suggestion must maintain or improve performance.
```

### **5. Documentation Currency**
```markdown
AI assistants MUST maintain documentation integrity:
✅ Update docs/CURRENT_STATUS.md with every significant change
...
❌ Traditional CRM features (pipelines, deal tracking, sales workflows)
❌ Social network features (posts, feeds, likes, comments, shares)
❌ Communication tools (chat, email, video, messaging replacement)
❌ Project management (tasks, deadlines, collaboration tools)
...
```markdown
NEVER suggest or implement:
❌ Code that slows Oracle Engine response times
❌ Database schemas that aren't relationship-centric
❌ API endpoints that don't serve Oracle intelligence
❌ UI components that don't advance "feels like future" standard
❌ Features that require manual data entry vs AI automation

Always check: Does this make relationship intelligence more magical?
```

### **Methodology Violations (Process Integrity)**
```markdown
NEVER:
❌ Skip 5-test framework validation for any feature
❌ Bypass documentation updates for significant changes
❌ Ignore Virtual Executive Team consultation for major decisions
❌ Suggest features without Revolutionary Standards validation
❌ Recommend shortcuts that compromise quality gates

Process integrity ensures long-term success and team coordination.
```

---

## 🔒 **MANDATORY AI VALIDATION FRAMEWORK**
...
5. Documentation Requirements
   ...
   □ Future AI context preserved

ONLY proceed after ALL validations pass.
```

### **Virtual Executive Team Escalation**
```markdown
Consult Virtual Executive Team for:
- Strategic feature decisions (@reid for strategy)
- Technical architecture choices (@jensen for tech)
- Business model changes (@mark for business)
- User experience decisions (@tim for operations)
...
## ⚙️ **AI WORKFLOW CONSTRAINTS**

### **Phase-Specific Constraints**

#### **Development Phase Constraints**
```markdown
Week 1 (Foundation): Focus ONLY on basic Oracle Engine and database
Week 2 (AI Intelligence): Focus ONLY on AI capabilities and relationship mapping
Week 3 (Business Features): Focus ONLY on B2B SaaS and team features
Week 4 (Launch): Focus ONLY on deployment and customer acquisition

NEVER mix phase objectives or suggest out-of-phase features.
```

#### **Feature Development Constraints**
```markdown
1. ONE feature at a time - no combined development
2. Complete feature before starting next one
3. Test and validate before moving forward
4. Update documentation immediately after completion
5. Performance validation before any deployment

Parallel development only for completely independent features.
```

#### **Quality Assurance Constraints**
```markdown
Mandatory for every feature:
□ Passes 5-test framework validation
□ Maintains <10 second Oracle performance
...
```markdown
1. POLITELY explain why the request violates constraints
2. REFERENCE specific constraint and reasoning
3. SUGGEST alternative approaches that align with mission
4. CONSULT Virtual Executive Team if needed (@exec)
...
- User insists on constraint violations
- Technical architecture decisions exceed AI knowledge
...
## 📊 **AI PERFORMANCE MONITORING CONSTRAINTS**
...
```markdown
AI assistants must:
□ Learn from constraint violations and prevent recurrence
□ Improve suggestion quality based on user feedback
...
```markdown
ALWAYS:
✅ Reference specific constraints when declining suggestions
✅ Explain reasoning with Revolutionary Standards context
...
❌ Suggest workarounds that violate principles
...
- Why constraints exist and their importance
- How constraints ensure revolutionary standards
...
## ⚖️ **CONSTRAINT VIOLATION CONSEQUENCES**

### **For AI Assistants**
```markdown
Constraint violations require:
1. Immediate correction and acknowledgment
2. Explanation of proper approach
3. Documentation of violation and prevention
...
```markdown
When users request constraint violations:
1. Education about constraint importance
2. Alternative solution suggestions
3. Virtual Executive Team consultation if needed
