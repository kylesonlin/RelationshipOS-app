-- Performance optimization: Fix slow queries with optimized functions and indexes

-- 1. Optimize our dashboard function to use better query patterns  
...
-- 2. Grant execute permission to authenticated users only
...
-- 3. Add indexes to reduce query execution times
CREATE INDEX IF NOT EXISTS idx_contacts_user_last_contact_simple 
ON contacts("userId", last_contact_date);

CREATE INDEX IF NOT EXISTS idx_tasks_user_status_created_simple 
ON tasks("userId", status, created_at);

CREATE INDEX IF NOT EXISTS idx_calendar_events_user_start_simple 
ON calendar_events(user_id, start_time);

-- 4. Update our existing function to be an alias to the optimized one
...
-- 5. Add additional performance indexes for common query patterns
CREATE INDEX IF NOT EXISTS idx_email_interactions_user_sent_simple 
ON email_interactions(user_id, sent_at);

CREATE INDEX IF NOT EXISTS idx_contact_activities_user_date_simple 
ON contact_activities(user_id, activity_date);

CREATE INDEX IF NOT EXISTS idx_user_integrations_user_active_simple 
