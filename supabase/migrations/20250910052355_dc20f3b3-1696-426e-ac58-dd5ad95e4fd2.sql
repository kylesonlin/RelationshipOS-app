-- Performance optimization: Fix slow queries with optimized indexes and functions
...
-- 3. Add indexes to reduce query execution times (using IF NOT EXISTS to avoid conflicts)
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_email_interactions_user_sent_at 
ON email_interactions(user_id, sent_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contact_activities_user_date 
ON contact_activities(user_id, activity_date DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_integrations_user_active 
